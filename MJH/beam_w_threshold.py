...
class SimConfig:
    ...
    # Proposed graph construction options.
    # binary: original paper rule A_ij = 1{B_ij > 0}
    # weighted-count: W_ij = w_aa*Baa_ij + w_am*(Bai_ij + Bia_ij), A_ij = 1{W_ij > threshold}
    # weighted-power: same idea, but overlap is weighted by beam powers.
    proposed_graph: str = "binary"
    w_active_active: float = 2.0
    w_active_moderate: float = 1.0
...
def build_weighted_beam_interference_count(
    Ba: np.ndarray,
    Bi: np.ndarray,
    w_active_active: float = 2.0,
    w_active_moderate: float = 1.0,
    ...
    """
    Count-based weighted beam interference matrix for the modified Proposed scheme.

    Baa = B^(a)^T B^(a): active-active overlap count.
    Bai = B^(a)^T B^(i): active-moderate overlap count.
    Bia = B^(i)^T B^(a): moderate-active overlap count.

    W = w_active_active * Baa + w_active_moderate * (Bai + Bia)
    """
    Baa = Ba.T @ Ba
    Bai = Ba.T @ Bi
    Bia = Bi.T @ Ba
    W = (
        w_active_active * Baa.astype(float)
        + w_active_moderate * (Bai.astype(float) + Bia.astype(float))
    )
    np.fill_diagonal(W, 0.0)
    return W, Baa, Bai, Bia


def build_weighted_beam_interference_power(
    cfg: SimConfig,
    stats: ChannelStats,
    active: np.ndarray,
    moderate: np.ndarray,
    ...
    """
    Power-weighted beam interference matrix for the modified Proposed scheme.

    This version reduces the effect of very weak moderate beams.  It uses
    sqrt(power_i * power_j) over common active/moderate beam positions.
    """
    K, L, N = cfg.K, cfg.L, cfg.N
    P = stats.beam_power.reshape(K, L * N).T.astype(float)  # [LN, K]
    A = active.reshape(K, L * N).T.astype(float)             # [LN, K]
    I = moderate.reshape(K, L * N).T.astype(float)           # [LN, K]
    ...
    Pi = P * I
    W = (
        w_active_active * (np.sqrt(Pa).T @ np.sqrt(Pa))
        + w_active_moderate * (np.sqrt(Pa).T @ np.sqrt(Pi))
        + w_active_moderate * (np.sqrt(Pi).T @ np.sqrt(Pa))
    )
    np.fill_diagonal(W, 0.0)
    return W


def adjacency_from_weighted_interference(W: np.ndarray, threshold: float) -> np.ndarray:
    """
    Thresholded adjacency matrix.

    A_ij = 1 if W_ij > threshold, otherwise 0.
    """
...
def proposed_assignment(cfg: SimConfig, stats: ChannelStats) -> Assignment:
    ...
    The rest of the simulator keeps the comparison schemes unchanged.
    Only the Proposed graph construction is switchable through cfg.proposed_graph:

      binary         : original paper rule, A_ij = 1{B_ij > 0}
      weighted-count : count-based weighted threshold graph
      weighted-power : power-weighted threshold graph
    """
    ...
    if cfg.proposed_graph == "binary":
        # Original paper rule:
        # B = B(a)^T B(a) + B(a)^T B(i) + B(i)^T B(a)
        # A_ij = 1 if B_ij > 0.
        B_interf = Ba.T @ Ba + Ba.T @ Bi + Bi.T @ Ba
        adjacency = B_interf > 0
        np.fill_diagonal(adjacency, False)
        graph_weights = B_interf.astype(float)

    elif cfg.proposed_graph == "weighted-count":
        # Count-based weighted graph:
        # active-active overlap receives larger weight than active-moderate overlap.
        graph_weights, _, _, _ = build_weighted_beam_interference_count(
            Ba,
            Bi,
            w_active_active=cfg.w_active_active,
            w_active_moderate=cfg.w_active_moderate,
        )
        adjacency = adjacency_from_weighted_interference(
            graph_weights, cfg.weight_threshold)

    elif cfg.proposed_graph == "weighted-power":
        # Power-weighted graph:
        # weak beam overlaps contribute less than strong beam overlaps.
        graph_weights = build_weighted_beam_interference_power(
            cfg,
            stats,
            active,
            moderate,
            w_active_active=cfg.w_active_active,
            w_active_moderate=cfg.w_active_moderate,
            normalize_per_user=cfg.normalize_power_weights,
        )
        adjacency = adjacency_from_weighted_interference(
            graph_weights, cfg.weight_threshold)
        ...
        raise ValueError(
            "cfg.proposed_graph must be one of: binary, weighted-count, weighted-power"
        )
    ...
    # Keep the name as 'Proposed' so plotting/printing code and comparison schemes remain unchanged.
    # assignment.weights stores either the original B matrix or the weighted W matrix for diagnostics.
    return Assignment(
        "Proposed",
        active,
        moderate,
...
def parse_args() -> argparse.Namespace:
    ...
    # Proposed graph options. These change only the Proposed scheme graph construction.
    p.add_argument("--proposed-graph", choices=["binary", "weighted-count", "weighted-power"], default="binary",
                   help="Proposed graph rule: binary is the original paper; weighted-count/power use thresholded weighted graph.")
    p.add_argument("--w-aa", type=float, default=2.0,
                   help="Weight for active-active beam overlap in weighted Proposed graph.")
    p.add_argument("--w-am", type=float, default=1.0,
                   help="Weight for active-moderate and moderate-active overlap in weighted Proposed graph.")
    p.add_argument("--weight-threshold", type=float, default=0.0,
                   help="Adjacency threshold for weighted Proposed graph: A_ij=1 if W_ij > threshold.")
    p.add_argument("--no-normalize-power-weights", action="store_true",
                   help="Disable per-user normalization in weighted-power Proposed graph.")
...
def cfg_from_args(args: argparse.Namespace, K: Optional[int] = None, N: Optional[int] = None, seed_offset: int = 0) -> SimConfig:
    return SimConfig(
        ...
        proposed_graph=args.proposed_graph,
        w_active_active=args.w_aa,
        w_active_moderate=args.w_am,
        weight_threshold=args.weight_threshold,
        normalize_power_weights=not args.no_normalize_power_weights,
