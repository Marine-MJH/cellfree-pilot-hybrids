...
Default environment in this variant:
  L=200 APs, K=50 UEs, fc=3 GHz, tau_c=150, fixed baseline tau_p=15.
  Weighted beam graph defaults: w_aa=2, w_ai=1, w_ia=1, edge_threshold=10.
  Antenna sweep: N-list = [1, 2, 4, 8].
  User sweep: K-list = [25, 30, 35, 40, 45, 50].
...
Additional AP-domain hybrids in this variant:
  ...
  gao                : Gao-style AP-domain many-to-many matching + matching-group pilot assignment.
...
class SimConfig:
    ...
    w_aa: float = 2.0  # active-active beam overlap weight
    w_ai: float = 1.0  # active-moderate beam overlap weight
    w_ia: float = 1.0  # moderate-active beam overlap weight
    edge_threshold: float = 10.0  # create edge if weighted overlap > edge_threshold
    ...
    power_control: str = "full"  # maxmin or full
...
def gao_matching_assignment(cfg: SimConfig, stats: ChannelStats, rng: np.random.Generator) -> Assignment:
    """Gao-style AP-domain matching baseline.

    H2GaoGreedy below is a hybrid: it uses Gao-style AP matching for grouping,
    then replaces the pilot choice with beta-dot-beta greedy coloring. This
    baseline keeps Gao's AP-domain matching output as the serving mask and uses
    matching groups directly for pilot assignment.
    """
    ap_mask_ref, _, _, _, _ = ap_domain_hybrid_masks(cfg, stats)
    tau_p = cfg.baseline_tau_p
    mu = many_to_many_ap_matching(cfg, stats, ap_mask_ref, ap_quota=tau_p)
    active = active_mask_from_ap_selection(cfg, mu)
    moderate = np.zeros_like(active, dtype=bool)
    reported = active.copy()
    n_rf = rf_chains_for_assignment(cfg, active, proposed=False)
    ...
    group_scores = group_scores_from_ratio(mu, ratio)
    group_order = np.argsort(-group_scores)
    pilots = np.full(cfg.K, -1, dtype=int)
    pilot_list = np.arange(tau_p, dtype=int)
    ...
    for ap in group_order:
        ...
        available = [p for p in pilot_list.tolist() if p not in occupied]
        unassigned = [int(u) for u in group.tolist() if pilots[u] < 0]
        if not unassigned:
            continue
        if len(available) >= len(unassigned):
            chosen = rng.choice(np.asarray(available, dtype=int), size=len(unassigned), replace=False)
            pilots[np.asarray(unassigned, dtype=int)] = chosen.astype(int)
        else:
            for u in rng.permutation(np.asarray(unassigned, dtype=int)):
                pilots[int(u)] = int(rng.choice(pilot_list))

    missing = np.flatnonzero(pilots < 0)
    if missing.size:
        pilots[missing] = rng.integers(0, tau_p, size=missing.size, endpoint=False)

    adjacency = ap_overlap_adjacency(mu)
    weights = common_serving_ap_ratio(mu)
    return Assignment(
        "GaoMatching",
        active,
        moderate,
...
SCHEME_ORDER = [
    "Random", "GaoMatching", "Greedy", "WGF", "GC",
...
def build_assignment_for_scheme(cfg: SimConfig, stats: ChannelStats, scheme: str, rng: np.random.Generator) -> Assignment:
    ...
    if sl in {"gaomatching", "gao-matching", "gao_matching", "gao"}:
        return gao_matching_assignment(cfg, stats, rng)
...
def _run_setup_job(cfg: SimConfig, setup: int, requested: Tuple[str, ...]) -> Tuple[int, Dict[str, SetupSchemeResult]]:
    return setup, run_all_schemes_for_setup(cfg, setup, requested)
...
def run_simulation(
    ...
    setup_results: List[Tuple[int, Dict[str, SetupSchemeResult]]] = []
    worker_count = max(1, int(workers))
    if worker_count > 1 and n_setups > 1:
        worker_count = min(worker_count, n_setups)
        if verbose:
            print(f"Running {n_setups} setups with {worker_count} worker processes", flush=True)
        with ProcessPoolExecutor(max_workers=worker_count) as executor:
            futures = {
                executor.submit(_run_setup_job, cfg, setup, requested_tuple): setup
                for setup in range(n_setups)
            }
            for done_count, future in enumerate(as_completed(futures), start=1):
                setup_idx, results = future.result()
                setup_results.append((setup_idx, results))
                if verbose:
                    print(f"setup {setup_idx + 1:03d}/{n_setups} done ({done_count}/{n_setups})", flush=True)
        setup_results.sort(key=lambda item: item[0])
    else:
        for setup in range(n_setups):
            results = run_all_schemes_for_setup(cfg, setup, requested_tuple)
            setup_results.append((setup, results))
            if verbose:
                print(f"setup {setup+1:03d}/{n_setups}", flush=True)
                for name in requested:
                    r = results[name]
                    print(
                        f"  {name:8s}: tau_p={r.assignment.tau_p:2d}, "
                        f"edges={int(np.sum(r.assignment.adjacency) // 2):4d}, "
                        f"RF/AP={float(np.mean(r.assignment.n_active_rf_per_ap)):5.2f}, "
                        f"avgSE={np.mean(r.se):.4f}, EE={r.ee:.6f}",
                        flush=True,
                    )

    for _, results in setup_results:
        for name in requested:
            r = results[name]
...
def plot_ecdf(metrics_by_scheme: Dict[str, SchemeMetrics], out_file: Optional[str] = None, title: str = "eCDF of per-user SE") -> None:
    if plt is None:
...
def plot_pilot_tau_boxplot(
    ...
    # Matplotlib compatibility: newer versions prefer tick_labels, while
    # older versions only support labels. This affects plotting only.
    try:
        plt.boxplot(data, positions=positions, tick_labels=labels, showmeans=True)
    except TypeError:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Environment-fixed all-scheme simulator: L=200, K=50, fc=3GHz, tau_c=150, tau_p=15 defaults")
    p.add_argument("--mode", choices=["fig1", "sweep-n", "sweep-k", "all", "single"], default="all")
    p.add_argument("--schemes", nargs="+", default=["random", "gao", "greedy", "wgf", "gc", "topap-bisect", "h2", "h3", "h4", "proposed", "matching-fixed", "matching-adaptive"], help="Schemes to simulate")
    p.add_argument("--L", type=int, default=200)
    p.add_argument("--K", type=int, default=50)
    p.add_argument("--N", type=int, default=8)
    p.add_argument("--K-list", type=int, nargs="+", default=[25, 30, 35, 40, 45, 50])
    p.add_argument("--N-list", type=int, nargs="+", default=[1, 2, 4, 8])
    p.add_argument("--setups", type=int, default=10, help="Paper uses 200; default is smaller for testing")
    p.add_argument("--workers", type=int, default=1, help="Number of worker processes for setup-level parallel simulation")
    p.add_argument("--seed", type=int, default=7)
    p.add_argument("--delta", type=float, default=95.0)
    p.add_argument("--baseline-tau-p", type=int, default=15)
    p.add_argument("--fc-ghz", type=float, default=3.0, help="Carrier frequency in GHz")
    p.add_argument("--tau-c", type=int, default=150, help="Coherence block length")
    p.add_argument("--beam-detect-snr-db", type=float, default=0.0)
    p.add_argument("--w-aa", type=float, default=2.0, help="Weight for active-active beam overlap in proposed/matching graphs")
    ...
    p.add_argument("--edge-threshold", type=float, default=10.0, help="Create interference edge if weighted overlap is greater than this threshold")
    ...
    p.add_argument("--power-control", choices=["maxmin", "full"], default="full")
...
def canonical_schemes(schemes: Sequence[str]) -> List[str]:
    ...
    for s in schemes:
        ...
        elif sl in {"gao", "gao-matching", "gao_matching", "gaomatching"}:
            names.append("GaoMatching")
...
def run_fig1(args: argparse.Namespace, schemes: List[str], outdir: str) -> Dict[str, SchemeMetrics]:
    cfg = cfg_from_args(args, K=args.K, N=args.N, seed_offset=0)
    metrics = run_simulation(cfg, args.setups, schemes, verbose=not args.quiet, workers=args.workers)
    print_metrics_table(metrics, cfg)
    if args.plot:
        plot_ecdf(metrics, os.path.join(outdir, "fig1_ecdf_vs_se_all_schemes.png"), title=f"eCDF of per-user SE, K={cfg.K}, N={cfg.N}")
    maybe_plot_pilot_boxplot(
        ...
        outdir,
        tag="fig1",
        title=f"Pilot count distribution, Fig.1/custom setting (K={cfg.K}, N={cfg.N})",
    )
    return metrics
...
def run_sweep_n(args: argparse.Namespace, schemes: List[str], outdir: str) -> List[Tuple[int, Dict[str, Tuple]]]:
    ...
    for idx, N in enumerate(args.N_list):
        print(f"\n######## Sweep N={N} ########")
        cfg = cfg_from_args(args, K=args.K, N=N, seed_offset=1000 * idx)
        metrics = run_simulation(cfg, args.setups, schemes, verbose=not args.quiet, workers=args.workers)
        print_metrics_table(metrics, cfg)
        ...
        maybe_plot_pilot_boxplot(
            ...
            outdir,
            tag=f"sweep_N_{N}",
            title=f"Pilot count distribution, N={N} (K={args.K})",
        )
    if args.plot:
...
def run_sweep_k(args: argparse.Namespace, schemes: List[str], outdir: str) -> List[Tuple[int, Dict[str, Tuple]]]:
    ...
    for idx, K in enumerate(args.K_list):
        print(f"\n######## Sweep K={K} ########")
        cfg = cfg_from_args(args, K=K, N=args.N, seed_offset=1000 * idx)
        metrics = run_simulation(cfg, args.setups, schemes, verbose=not args.quiet, workers=args.workers)
        print_metrics_table(metrics, cfg)
        ...
        maybe_plot_pilot_boxplot(
            ...
            outdir,
            tag=f"sweep_K_{K}",
            title=f"Pilot count distribution, K={K} (N={args.N})",
        )
    if args.plot:
...
def main() -> None:
    ...
    if args.mode == "fig1":
        ...
        print_metrics_table(metrics, cfg)
        if args.plot:
            plot_ecdf(metrics, os.path.join(outdir, "single_ecdf_all_schemes.png"), title=f"eCDF of per-user SE, K={cfg.K}, N={cfg.N}")
        maybe_plot_pilot_boxplot(
            args,
            metrics,
            schemes,
