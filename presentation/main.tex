...
\begin{frame}{Reproduction Anchor: Gao Small-Pilot Regime}
    ...
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.40\textwidth}
            ...
            \envnote{Environment: Gao reproduction setting; different from the same-environment multi-antenna results.}
        \end{column}
        \begin{column}{0.56\textwidth}
            \centering
...
\begin{frame}{Main Result: User Load Sweep}
    \small
    \centering
    ...
    \begin{columns}[T,onlytextwidth]
        ...
        \begin{column}{0.49\textwidth}
            ...
            \textbf{Result}
            \begin{tightitemize}
                \item Gao stays near Random.
                \item Top-N and Resource stay above Random through $K=50$.
                \item Beam-Weighted crosses below Random from $K=40$.
                \item At $K=50$: Resource $+6.66\%$, Top-N $+5.35\%$, Mussbah $-23.24\%$.
            \end{tightitemize}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Energy Efficiency Under User Load}
    \small
    \centering
    \includegraphics[width=0.90\linewidth,height=0.56\textheight,keepaspectratio]{presentation_6method/presentation_clean_load_crossover_ee.png}

    \vspace{0.15em}
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.47\textwidth}
            \scriptsize
            \textbf{Environment:} same K-sweep as the SE slide; EE is a proxy using the same selected method set.
        \end{column}
        \begin{column}{0.49\textwidth}
            \scriptsize
            \textbf{Result}
            \begin{tightitemize}
                \item Beam-Resource Matching stays highest across the sweep.
                \item At $K=50$: Resource $+12.35\%$, AP-Top-N $+5.29\%$.
                \item Mussbah loses EE under load as its pilot count grows.
            \end{tightitemize}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Conclusion}
    ...
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.48\textwidth}
            ...
            \begin{tightitemize}
                \item Beam-Resource and AP-Top-N keep positive SE and EE margins.
                \item Beam-Weighted crossover shows pilot-budget control matters.
                \item Gao is a reproduction anchor, not the main winner here.
            \end{tightitemize}
        \end{column}
        \begin{column}{0.48\textwidth}
            \scriptsize
            ...
            \begin{tightitemize}
                ...
                \item $\taup=3$ stress is diagnostic.
                \item SE gain is not direct evidence of higher SINR.
    ...
    \begin{center}
        \footnotesize
        \textbf{Final claim:} adaptive pilot assignment is useful when it cuts pilot overhead without visibly sacrificing SINR.
    \end{center}
\end{frame}
